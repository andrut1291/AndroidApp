...
public class ChartFromButton2 extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    Number[] series1Numbers = new Number[900];
	    for (int i = 0; i < series1Numbers.length; i++) {
			series1Numbers[i] = i;
		}
	    
	    Number[] series2Numbers = new Number[900];
	    Random generator = new Random();
	    for (int i = 0; i < series2Numbers.length; i++) {
			series2Numbers[i]=generator.nextInt(2);
		}
		XYSeries series2 = new SimpleXYSeries(time,data, "Wartość napięcia");
		...
		mySimpleXYPlot.setDomainStep(XYStepMode.SUBDIVIDE,series1Numbers.length);
		mySimpleXYPlot.setTicksPerDomainLabel(series1Numbers.length/6);
		...
		try {
			...
			for (int i = 0; i < jsonState.length(); i++) {
				...
				if(value.contentEquals("0")){
					...
					Toast.makeText(getApplicationContext(), "Zły format danych na serwerze",
